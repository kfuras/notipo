...
async function auth(app: FastifyInstance) {
  ...
  app.addHook("onRequest", async (request: FastifyRequest, reply) => {
    ...
    if (request.url === "/health" || request.url === "/favicon.ico" || request.url === "/api/notion/webhook" || request.url.startsWith("/api/notion/oauth/callback") || request.url.startsWith("/api/auth/") || request.url.startsWith("/api/uploads/") || request.url.startsWith("/api/default-category-images/")) return;
